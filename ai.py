...
def get_the_best_policy(board,player_flag,opt_func = lambda x : max(x),):
    ...
    for policy in possible_policies:
        new_board = new_board(board,policy)
...
def max_utility(board, player):
    ...
    if state[0] != 1 or state[1] != 0:
        ...
        for action in available_policy(board):
            value = max(value, min_utility(
                core.new_board(board, action, player), player))
        return value

def min_utility(board, player):
    """
